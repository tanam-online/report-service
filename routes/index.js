...
*/
router.get('/download/:id/:field/:timeStart/:timeEnd', async (req, res) => {
  try {
    if (!req.params.id) {
    ...
    if (!req.params.timeStart || !req.params.timeEnd) {
      response = await axios.get(`http://api-monitoring-tanam.herokuapp.com/real-time/${req.params.id}/${req.params.field}`)
    } else {
      response = await axios.get(`http://api-monitoring-tanam.herokuapp.com/real-time/${req.params.id}/${req.params.field}/${timeStart}/${timeEnd}`)
    }
